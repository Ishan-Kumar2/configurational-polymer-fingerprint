...
double DeltaEnergy(Bead& bn, vector<Bead>& b, Input& in) {
  ...
  for (int i = 0; i < int(bn.nbr.size()); i++)
    temp +=
        Elastic(bn, b[bn.nbr[i]], in) - Elastic(b[bn.index], b[bn.nbr[i]], in);
  return temp;
}
...
double TotalEnergy(vector<Bead>& b, Input& in) {
  ...
  for (int i = 0; i < in.NBEAD; i++) {
    for (int j = 0; j < int(b[i].nbr.size()); j++)
      if (b[i].nbr[j] > i) temp += Elastic(b[i], b[b[i].nbr[j]], in);
  }
  return temp;
}
...
double calcLJ(double dist) {
  if (dist > pow(2, 1 / 6)) {
    ...
    return 4.0 * ((1.0 / pow(dist, 6)) - (1.0 / pow(dist, 3)));
}
...
double x, y, z;
vector<double> calcRg3D(vector<Bead>& pos, Input& in, double& r_cm_x,
                        double& r_cm_y, double& r_cm_z) {
  ...
  vector<double> r{evalues(0, 0), evalues(1, 0),
                   trace - (evalues(1, 0) + evalues(0, 0))};
